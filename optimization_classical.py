...
#Here we find the classical optimization method based on the distribution p_{\hat{X}, \hat(Y)}
...
if __name__ == "__main__":
    ...
    def Delta(proba):
        return cp.sum(cp.abs(proba - p_x_y_const)) / 2
    ...
    objective = cp.Minimize(Delta(P_cond@p_x_y_d_const))
                    ...
                    for m in range(d_dim):
                        for h in range(age_dim.shape[0]):
                            for b in range(charge_dim.shape[0]):
                                for c in range(priors_dim.shape[0]):
                                    for y in range(y_dim):
                                        P_to_save[h,b,c,y,i,j,k,l,m] = P_cond_opt[direct_index_1(h,b,c,y),direct_index_2(i,j,k,l,m)]




    P_to_save = np.transpose(P_to_save, (0, 1, 2, 9, 4, 5, 6, 8, 7))
    np.save('P_cond_optimized_classical.npy', P_to_save)
